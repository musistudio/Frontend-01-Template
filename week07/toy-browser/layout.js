...
function layout(element) {
  ...
  var items = elemnet.children.filter((e) => e.type === 'element');
  ...
  for (var i = 0; i < item.length; i++) {
    ...
    itemStyle[mainSize] === null && (itemStyle[mainSize] = 0);
    if (itemStyle.flex) {
      flexLine.push(item);
  ...
  flexLines.forEach((items) => {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      if (itemStyle[crossSize] === 'flex-start') {
        itemStyle[crossStart] = crossBase;
        itemStyle[crossEnd] =
          itemStyle[crossStart] + crossSign * itemStyle[crossSize];
      }

      if (itemStyle[crossSize] === 'flex-end') {
        itemStyle[crossEnd] = crossBase + crossSign * lineCrossSize;
        itemStyle[crossStart] =
          itemStyle[crossEnd] - crossSign * itemStyle[crossSize];
      }

      if (itemStyle[crossSize] === 'center') {
        itemStyle[crossStart] =
      ...
      if (itemStyle[crossSize] === 'stretch') {
        itemStyle[crossStart] = crossBase;
        itemStyle[crossEnd] =
          crossBase +
          crossSign *
